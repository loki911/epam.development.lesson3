...
public class ApplianceFactoryImpl implements ApplianceFactory {
    ...
    private Map<String, String> stingToKeyValueProps(String value) {
        // Strip optional semicolon at the end of line
        ...
        for (String keyValue: pairs) {
            ...
            if (propData.length != 2) {
                throw new DAOException(
                        String.format("Invalid property '%s': must be two strings " +
                                "separated by exactly one equal sign", keyValue));
            }
            result.put(propData[0].trim(), propData[1].trim());
        }
